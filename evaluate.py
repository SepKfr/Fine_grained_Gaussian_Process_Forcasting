...
mse_std = torch.mean(mse_std.std(dim=0)).item() / np.sqrt(1)
mae_std = torch.mean(mae_std.std(dim=0)).item() / np.sqrt(1)
...
error_path = "4th_final_errors_{}_{}.json".format(args.exp_name, pred_len)
