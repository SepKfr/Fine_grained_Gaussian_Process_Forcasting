...
for key in mses.keys():
    ...
    plt.plot(np.arange(total_steps - pred_len), tgt[key][:-pred_len], color="black", alpha=0.5)
    plt.plot(np.arange(total_steps - pred_len, total_steps), tgt[key][-pred_len:], color="gray", alpha=0.5)
    plt.plot(np.arange(total_steps - pred_len, total_steps), preds[key], color="lime")
    ...
    plt.plot(np.arange(total_steps - pred_len), tgt[key][:-pred_len], color="black", alpha=0.5)
    plt.plot(np.arange(total_steps - pred_len, total_steps), tgt[key][-pred_len:], color="gray", alpha=0.5)
    plt.plot(np.arange(total_steps - pred_len, total_steps), preds_random[key], color="orchid")
    plt.axvline(x=total_steps - pred_len, color="black", linestyle='--')
    ...
    plt.plot(np.arange(total_steps - pred_len), tgt[key][:-pred_len], color="black", alpha=0.5)
    plt.plot(np.arange(total_steps - pred_len, total_steps), tgt[key][-pred_len:], color="gray", alpha=0.5)
    plt.plot(np.arange(total_steps - pred_len, total_steps), preds_gp[key], color="darkblue")
    plt.axvline(x=total_steps - pred_len, color="black", linestyle='--')
